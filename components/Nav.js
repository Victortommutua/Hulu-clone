    ...
    <nav className="relative">
        ...
        sm:space-x-20 overflow-x-scroll scrollbar-hide">
            {Object.entries(requests).map(([key,{title,url}])=>(
                <h2 key={key} onClick={()=> router.push(`/?genre=${key}`)} className = 'cursor-pointer transition-duration-100 transform-hover:scale-125 hover:text-white active:text-red-500' 
                >{title}</h2>
            ))}
        </div>
